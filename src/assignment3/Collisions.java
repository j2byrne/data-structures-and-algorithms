...
public class Collisions {
	public static void polynomiala(int a) throws FileNotFoundException {
		...
		HashMap<Integer, Integer> hmap = new HashMap<Integer, Integer>(); // hashmap to store hashcode and number of collisions
		
		while (in.hasNext()) {
			String str = in.next();
			
			// calculate hashcode for the word
			int hash = 0;
			...
			if (hmap.containsKey(hash)) {
				collisions += 1; // add one to the collisions variable
			} else {
				hmap.put(hash, null); // add the key to the hashmap
			}
		}
		in.close();
	...
	public static void main(String[] args) throws FileNotFoundException {
		polynomiala(41); // 4 collisions
		polynomiala(17); // 387 collisions
		polynomialCyclic(7); // 290 collisions
		polynomialOld(); // 147 collisions
	}
}
